...
以上参数可以修改到 `/etc/sysctl.conf` 文件中；是否使用Keepalive用来保活就够了，其实还不够，Keepalive只是在网络层就行保活，如果网络本身没有问题，但是系统由于其他原因已经不可用了，这时候Keepalive并不能发现；所以往往还需要结合心跳机制来一起使用；
...
```
heartbeat默认为60秒，heartbeatTimeout默认为 `heartbeat*3`，可以理解至少出现三次心跳请求还未收到回复才会任务连接已经断开；HeartBeatTask为执行心跳的任务：
```
public void run() {
...
```
同上检测最后读写时间和heartbeat的大小，注：普通请求和心跳请求都会更新读写时间；[Netty 在 Dubbo 中是如何应用的？](https://mp.weixin.qq.com/s?__biz=MzI3ODcxMzQzMw==&mid=2247491700&idx=3&sn=62fd8c9bd68aeac3cd1b98f621a2b1e7&chksm=eb506542dc27ec54ed9de0989fc4f74df79c902fae6fc6f11498b61eb714df33e724af1fd4ea&scene=21#wechat_redirect)这篇推荐大家看一下。
```
protected void doTask(Channel channel) {
    Long lastRead = lastRead(channel);
