...
6. **间隙锁(Gap Lock)** 
   ...
   假如数据库已有ID为1、6两条记录， 现在想要在ID in （4，10）之间更新数据的时候，会加上间隙锁，锁住[4,5] [7,10] ,(不包含已有记录ID=5本身) 。那么在更新ID=5的记录（只有一条记录）符合条件； 如果不加间隙锁，事务2有可能会在4、10之间插入一条数据，这个时候事务1再去更新，发现在(4,10)这个区间内多出了一条“幻影”数据。
