...
- **C** 事务的**一致性**(Consistency)：指事务的运行并不改变数据库中数据的一致性.例如,完整性约束了 `a+b=10`,一个事务改变了a,那么b也应该随之改变.
