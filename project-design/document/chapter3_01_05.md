...
说明：`ApiVersion` 为自定义的注解，`API` 版本控制，返回对应的版本号。
...
当方法级别和类级别都有 `ApiVersion` 注解时，二者将进行合并（`ApiVersionRequestCondition.combine`）。最终将提取请求 `URL` 中版本号，与注解上定义的版本号进行比对，判断 `url` 是否符合版本要求。
